...
my $fasta_file = Amphora2::Utilities::alignment_to_fasta($aln_file, $target_dir);

my $hmm_file = Amphora2::Utilities::generate_hmm( $aln_file, $target_dir );

Amphora2::Utilities::fasta2stockholm($aln_file , "$target_dir/$core.stk");
my $stk_aln =  "$target_dir/$core.stk";
#may need to create an unaligned file for the sequences before aligning them
my $new_alignment_file = Amphora2::Utilities::hmmalign_to_model( $hmm_file, $fasta_file, $target_dir, $stk_aln);
my $clean_alignment_file = Amphora2::Utilities::mask_and_clean_alignment( $new_alignment_file , $target_dir );
my ( $fasttree_file, $tree_log_file ) = Amphora2::Utilities::generate_fasttree( $clean_alignment_file, $target_dir );
...
my $rep_fasta = Amphora2::Utilities::get_fasta_from_pda_representatives( $rep_file, $target_dir, $fasta_file );
...
`cd $target_dir;taxit create -c -d "Creating a reference package for Amphora-2 for the $core marker" -l $core -f $target_dir/$core.masked -t $target_dir/$core.tree -s $target_dir/$core.log -Y FastTree -P $core`;
`rm $target_dir/$core.pda`;
`rm $target_dir/$core.tree`;
`rm $target_dir/$core.log`;
`rm $target_dir/$core.aln`;
`rm $target_dir/$core.fasta`;
`rm $target_dir/$core.masked`;
