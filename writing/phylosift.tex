...
PhyloSift also provides a means to visualize the relative abundance of taxonomic groups present in a sample.
We show two such plots for the samples 
...
The analysis of human fecal microbial communities we describe was possible with only 50 Mbp median depth of coverage per sample.
Current Illumina HiSeq 2000 instruments generate 38 Gbp per lane, suggesting that up to 760 samples could be processed in a single Illumina lane and yield similar findings.
Based on current Illumina sequencing service provider costs this suggests large metagenome surveys could be conducted for as little as to \$2.50 to \$5 per sample in sequencing costs.
