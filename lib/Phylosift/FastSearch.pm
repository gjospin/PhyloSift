...
sub get_hits_contigs {
	...
	while (<$HITSTREAM>) {
		...
		if ( defined( $contig_top_bitscore{$query} ) ) {
			...
			$hit_counts{$markerName}++;
			$hit_counts_total++;
			my @hitdata = [ $markerName, $bitScore, $query_start, $query_end, $frameshift, $pid ];
			push( @{ $contig_hits{$query} }, @hitdata );
...
sub write_candidates {
	...
	my $passing = 0;
	debug "CONTIG HITS : ".scalar(keys %contig_hits)."\n";
	...
	while ( my $seq = $seqin->next_seq ) {
		...
		for ( my $i = 0 ; $i < @{ $contig_hits{ $seq->id } } ; $i++ ) {
			...
			$markerLength{$markerHit} = Phylosift::Utilities::get_marker_length(          self   => $self,
																				 marker => $markerHit )
