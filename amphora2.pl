...
GetOptions(
	...
	"coverage=s"      => \$coverage,            #provides a contig/scaffold coverage file
	"updated_markers" => \$updated,             #Indicates if Amphora2 uses the updated versions of the Markers.
	"config=s"        => \$configuration,       #custom config file
...
$newObject->{"configuration"}    = $configuration if defined($configuration);
